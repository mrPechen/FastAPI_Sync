...
def get_menus2(db: Session):
    return db.query(func.count(models.Menu.submenu_rel)).count()
...
def create_dish(db: Session, dish: schemas.DishCreate, menu_id: int, submenu_id: int):
    db_dish = models.Dish(**dish.model_dump(), menu_id=menu_id, submenu_id=submenu_id)
    db.add(db_dish)
    db.commit()
    db.refresh(db_dish)
    return db_dish
